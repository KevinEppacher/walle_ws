...
class nMPC:
    def __init__(self, model):
        self.model = model
        # Prediction horizon
        self.N = rospy.get_param('nmpc_controller/N', 15)
        # Weighting matrices
        Q_param = rospy.get_param('nmpc_controller/Q', [100, 100, 0.001])
        R_param = rospy.get_param('nmpc_controller/R', [0.05, 0.05])
        S_param = rospy.get_param('nmpc_controller/S', [1, 1, 0.0])
        self.S = np.diag(S_param)
        self.Q = np.diag(Q_param)
        self.R = np.diag(R_param)
        # Maximum number of obstacles
        self.n_obstacles = rospy.get_param('nmpc_controller/max_obstacles', 10)
        # Control bounds
        self.v_min = rospy.get_param('nmpc_controller/v_min', 0.0)
        self.v_max = rospy.get_param('nmpc_controller/v_max', 0.2)
        self.omega_min = rospy.get_param('nmpc_controller/omega_min', -0.5)
        self.omega_max = rospy.get_param('nmpc_controller/omega_max', 0.5)
        # Prediction distance
        self.prediction_distance = rospy.get_param('nmpc_controller/prediction_distance', 2.0)
        self.robot_safety_diam = rospy.get_param('nmpc_controller/robot_safety_diam', 0.2)
        self.penalty_weight = rospy.get_param('nmpc_controller/penalty_weight', 0.6)
        self.epsilon = rospy.get_param('nmpc_controller/epsilon', 0.1)
    ...
    def define_optimization_problem(self):
        ...
        # Extract T from the parameter vector P (last entry in P)
        T = P[-1]
        ...
        for k in range(N-1):
            ...
            st_next_euler = st + T * f_value  # Use T from the parameter vector
            g = ca.vertcat(g, st_next - st_next_euler)
        
        # Add the terminal cost for the final state
        terminal_state = X[:, N]
        ...
        for k in range(N + 1):
            for i in range(self.n_obstacles):
                ...
                g = ca.vertcat(g, hard_obs_constraint)
        ...
        # Make decision variable a column vector
    ...
    def solve_mpc(self, current_state, ref_traj, target_state, T, obstacles):
        ...
        # Use each point in the reference trajectory
        for k in range(self.N):
            ...
            # Fill the args['p'] array with the reference values
        ...
        # Add T to the parameter vector (last entry in args['p'])
        args['p'][-1] = T
